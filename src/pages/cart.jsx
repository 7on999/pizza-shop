...
import {clearCartAC} from '../redux/reducers/cart';
import cartEmptyPng from '../assets/img/empty-cart.png';

const Card = ()=>{

const {totalCount, totalPrice, items, uniqPizzas} = useSelector(({cart})=>cart);

const dispatch = useDispatch();

const clearCart= ()=>{
  if (window.confirm('are you realy want to clear cart?')) {
...
{uniqPizzas.map(pizza=><CartItem  name={pizza.name} size={pizza.size} id={pizza.id}
 type={pizza.type} price={pizza.price} key={`${pizza.id}_${pizza.name}_${pizza.size}_${pizza.type}_${Math.random()*25}`} 
totalPizzas={items[pizza.id][pizza.size].filter(obj=>obj.type===pizza.type).length}
totalCost={items[pizza.id][pizza.size].filter(obj=>obj.type===pizza.type).reduce((sum, obj)=>
obj.price+sum, 0)}
 imageUrl={pizza.imageUrl}  rating={pizza.rating} category={pizza.category}
